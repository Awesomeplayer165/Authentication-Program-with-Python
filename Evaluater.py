...
class Evaluater:
	...
	@property
	def hasCapitalLetters(self) -> bool:
		return any(i.isupper() for i in self.password)

	@property
	def hasLowercaseLetters(self) -> bool:
		return any(i.islower() for i in self.password)

	@property
	def containsName(self) -> bool:
		return self.name.lower() in self.password.lower()

	@property
	def containsLiterallyPassword(self) -> bool:
		return self.password.lower() == "password"

	@property
	def containsUsername(self) -> bool: # Added
		return self.username.lower() in self.password.lower()

	@property
	def hasMoreThan100Characters(self) -> bool:
		return len(self.password) > 100
